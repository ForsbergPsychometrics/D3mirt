...
for critical value $z$. The critical value is indicated by the user using a percentage for the relevant confidence interval, e.g., 95 % CI (default `ci = 0.95`), and the critical value is calculated as approximately 1.96 if using two decimal places in the information function. Note that the number of decimals in the output can be controlled using the `digits` argument (the default is set to four digits).
