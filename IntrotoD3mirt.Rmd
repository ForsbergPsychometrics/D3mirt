...
```{r, include = FALSE}
# tools::compactPDF("IntrotoD3mirt.pdf", gs_quality = "ebook")
...
2.  `D3mirt` and `D2mirt` Model Estimation

-   2.1. The `D3mirt()` and `D2mirt` Function
    -   2.1.1. Calling the Compensatory Model
    -   2.1.2. Calling the Orthogonal Model
    -   2.1.3. Constructs by Items
    -   2.1.4. Constructs by Spherical or Polar Angles
...
```
```{r, eval = FALSE}
# Call to modid() using three factors (default)
id <- modid(x)
```

To identify a two-dimensional model (not shown here), just change the number of factors to `factors = 2`. 
```{r, eval = FALSE}
# Call to modid() using two factors
id <- modid(x, factors = 2)
```

To investigate the thee diemsnional output, use `summary`.
```{r}
summary(id)
```
...
The process for fitting a model with either `D3mirt()` or `D2mirt()` is the same with only minor exceptions. Foremost, `D2mirt()` only uses one model identification item, and adding a construct vector relies on polar angles; hence, only one angle is required (radial distance is constant). 
...
To fit a two-dimensional DMIRT model (not shown here), just use one model identification item. 
```{r, eval = FALSE}
# Call D2mirt() with one model identification item
mod1 <- D2mirt(x, modid = "W7Q10")
```

### 2.1.2. Calling the Orthogonal Model

An alternative option is to fit a constrained version of the compensatory model. This model allows the user to restrict items in the model space to only load on one of the model axes. This results in an orthogonal model which can be useful for studying the items under the assumption that the items are unidimensional and dimensions are uncorrelated. In this context *orthogonal* refers to the geographical interpretation of the graphical output in which all three unidimensions are perpendicular in the model space. Note that the discrimination parameter is reported as an $DISC$ index since it is no longer is optimized within the latent environment. Instead, the parameter is just a regular unidimensional discrimination index. 
...
To fit a two-dimensional DMIRT model (not shown here), use two nested item lists. 
```{r, eval = FALSE}
# misfitting item W7Q16 removed in data set y
mod2 <- D2mirt(y, modid = list(c(1:14),
                             c(15:19)))
```
...
The example below will only illustrate three-dimensional plotting since the example data is three dimensional. However, the general principles rae the same for pltting in two-dimensions. The only exception is the confidence interval which uses an standard deviation to add width to the elliptic disc in the two-dimensional plot. This is only for presentation purposes and holds no statistical meaning. 
...
Graphing in default mode by calling `plot()` will return an RGL device that will appear in the console as a three-dimensional interactive object that can be rotated manually by the user. In this illustration, however, all RGL devices are plotted as inline interactive objects with the help of R markdown. Note that it is possible to plot the RGL device in an external window by setting `console = FALSE`. 
...
![Compensatory Model 1 illustrating multidimensional item charachteristics rotated $15^{\circ}$ clockwise.](./images/mod1.png){width=100%}
...
![The Orthogonal, Model 2, with item vectors strictly paralell with the model axes, rotated $15^{\circ}$ clockwise.](./images/o1.png){width=100%}
...
To plot constructs alongside item vectors, change to `constructs = TRUE`. The constructs can be named with strings as input to the `construct.lab` argument. In this example, construct labels were chosen based on item content. Note that colors of item vectors can be changed with the `col` argument and colors of constructs with `c.col`.
...
![Model 3 rotated $15^{\circ}$ showing constructs (solid black arrows) based on item subsets.](./images/anes1.png){width=100%}
...
![Model 4 rotated $15^{\circ}$ showing constructs (solid black arrows) based on spherical coordinates.](./images/mod4.png){width=100%}
...
![Model 3 rotated $15^{\circ}$ showing items 15 to 20 and the three item subset constructs.](./images/con1.png){width=100%}
...
![Model 3 rotated $15^{\circ}$ showing items 15 and the three item subset constructs.](./images/item1.png){width=100%}
...
![Model 3 rotated $15^{\circ}$ showing all respondents trait scores illustrated as black spheres.](./images/mod3hide.png){width=100%}
...
![Model 3 rotated $15^{\circ}$ graphically separating male trait scores (blue) from female trait scores (red).](./images/p1.png){width=100%}

An example of how the output can be described is as follows:

> In Figure 10, a simple gender profile can be observed, showing that more women tend to have higher levels of compassion. When rotating the model $90^{\circ}$ clockwise, there seems to be no obvious gender difference related to Conformity or Fairness.
...
![Model 2, the orthogonal model, rotated $15^{\circ}$ showing respondents male trait scores (blue) and female trait scores (red).](./images/mod3blueredorth.png){width=100%}
...
![Model 3 rotated $15^{\circ}$ graphically separating respondents 30 years or younger (red) from respondents 70 years and older (blue).](./images/mod3age.png){width=100%}
...
![Model 3 rotated $15^{\circ}$ showing respondents 30 years or younger surrounded by a $95\, %$ CI.](./images/ci1.png){width=100%}
...
Information in IRT tells the quality of the test and item. In brief, higher amounts of information imply that the test or item is efficient in separating respondents on the latent trait. The information function displays item information, estimated in the `D3mirt()` and `D2mirt()` functions, respectively. Based on these estimates, confidence intervals are calculated, and basic descriptive statistics are presented.

Information is defined as 

$$I_i(\theta) := P_i(\theta) Q_i(\theta)a_i^2,$$

and taking measurement direction into account gives

$$I_i(\theta, \omega) := P_i(\theta) Q_i(\theta) \bigg( \sum^n a_i \cos \omega \bigg)^2.$$

Taking the inverse gives the standard error, defined as 

$$SE(\hat{a}_i) := \frac{1}{\sqrt{I(\hat{a}_i)}},$$

for discrimination estimator $\hat{a}_i$. The confidence intervals for the MDISC and DDISC parameters are calculated as 

$$CI = \hat{a}_i \pm z \times SE(\hat{a}_i),$$

for critical value $z$. The critical value is indicated by the user using a percentage for the relevant confidence interval, e.g., 95 % CI (default), and the critical value is calculated as approximately 1.96 if using two decimal places in the information function. Note that the number of decimals in the output can be controlled using the `digits` argument (the default is set to four digits).

```{r}
# 95 % CI is default
info <- information(mod3)
```

The output in this example provides the same model information as the summary and print methods for the S3 objects (`D3mirt` or `D2mirt`). This is followed by descriptives of the MDISC, including the total sum of information and the mean, standard deviation, and median of the same. Next, the mean, standard deviation, and median of the standard errors are reported. If constructs were included, as in the example above, in the model specification, DDISC descriptives are reported for each construct vector, following the same patterns as for MDISC.  

It is, however, important to note that MDISC estimates represent the maximum discrimination of an item at its optimal angle in the multidimensional space. The MDISC parameters are, therefore, dependent on measurement direction and consequently not comparable unless the items are oriented identically. Summarizing MDISC parameters can, therefore, result in misleading interpretations if assumed to represent a sum of unidimensional information. In contrast, DDISC parameters are projected onto a specific, fixed construct vector and are constrained to a single shared orientation. They can, therefore, can be compared and aggregated under the assumption of unidimensionality.
Note that the standard error calculation is capped to avoid extreme-value problems (see the manual for more information and options).

After the descriptives section, MDISC parameters per item are presented alongside item information, standard error, and confidence intervals calculated conditioned on the critical value set by the user. Because constructs were used in the model estimation, the same information is reported per item for each construct. The output provides useful information for comparing optimized parameters and their measurement characteristics (based on the MDISC) vs. the directional parameters (based on the DDISC). Foremost, the DDISC parameters show that the items' unidimensional capacity in the latent space is always lower than that of the DMIRT parameters, which relax the assumption of a single measurement continuum.

# 5. Exporting The RGL Device

Below are some options for exporting the RGL device. In addition, it is also possible to export graphical devices in R Markdown documents with `rgl::hook_webgl()` together with graphical options for knitr, as was done when creating this vignette. When exporting images, it is good to use the `view` argument when calling `plot()` to adjust the angle of the device captured in the image file. In the function calls below, the angle of the device is set to $45^{\circ}$ rotation in the plane, $20^{\circ}$ tilt away from the $y$-axis, and a zoom factor of $0.6$ (the default is `c(15, 20, 0.6)`).
...
5.  Add `technical = list(NCYCLES = 1000)` as an extra argument. 
6.  Change `type`or `c.type` to simpler arrow like `lines` in `plot().

Points 1 to 4 below regard issues with the `modid()` function specifically. Point 5, however, can be used on all functions that rely on `mirt()` for factor analysis. These functions are `modid()`, `D2mirt()`, and `D3mirt()`. More specifically, the function call resolves a warning from `mirt()` by providing more time to find a stable solution during the factor estimation process. Regarding point 6, sometimes RGL graphics throws an error due to arrow location in the model. A simple solution is to change from `rotation` in `type`or `c.type` when calling `plot()`, which has a simpler graphical structure and hence is more stable. 

Point 4 should, however, only be used as a last resort when calling `modid()`. This is because the upper bound sets the upper limit for item inclusion. Adjusting this limit too high compromises the necessary statistical requirements. The lower limit (point 2), however, only increases the size of the item pool used for the item selection. It is therefore recommended to adjust the lower limit up or down to see whether the output differs, and, based on that, make the final decision.
...
Regarding point 5, adding the `technical = list(NCYCLES = 1000)` argument is very straight forward. The example below illustrates the simple procedure on a call to `D3mirt()`.
```{r, eval = FALSE}
# Call D3mirt() with the model identification items
mod1 <- D3mirt(x, modid = c("W7Q3", "W7Q20"), technical = list(NCYCLES = 1000))
```
